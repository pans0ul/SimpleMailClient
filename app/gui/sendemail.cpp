...
void SendEmail::on_openFile_clicked()
{
    ...
    if (dialog.exec()) {
        ui->uFilePath->setText(dialog.selectedFiles().join(QLatin1String("; ")));
        //导入openFile打开的内容到textBrowser并显示
        QFile file(dialog.selectedFiles().first());
        if (file.open(QIODevice::ReadOnly)) {
            QTextStream in(&file);
            ui->utextBrowser->setText(in.readAll());
            file.close();
        }
    }

    //test print uDayValue and uHourValue
    // qDebug() << "uDayValue" << ui->uDayValue->text();
    // qDebug() << "uHourValue" << ui->uHourValue->text();

    // ui->ScheduleTimer->setText("Schedule Timer123");
